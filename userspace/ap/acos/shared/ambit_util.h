...
/* jack tseng 10/18/2017 add */
#ifdef ENABLE_UPDATE_DEVICE_INFO
#ifndef isxdigit
...
/* Foxconn add end, Jenny Zhao, 01/25/2011 */
    //LOG_CATEGORY_OPENVPN    = (1 << 9),   // JAG-205 There are many error log when failed to connect VPN connection.
    LOG_CATEGORY_READYSHARE = (1 << 9),
    LOG_CATEGORY_MOBILE     = (1 << 10),
    LOG_CATEGORY_VPNSERVICE = (1 << 11),
...
/* Foxconn add end, Jenny Zhao, 01/25/2011 */
    ...
    LOG_EVENT_USB_ATTACH         = (LOG_CATEGORY_READYSHARE << 16) | 31,
    LOG_EVENT_USB_DETACH         = (LOG_CATEGORY_READYSHARE << 16) | 32,
...
/*fxcn add by dennis end,11/16/2012,add WPS aplockdown log*/
    ...
    //LOG_EVENT_OPENVPN           = (LOG_CATEGORY_OPENVPN << 16) | 41,  // JAG-205 There are many error log when failed to connect VPN connection. 
    LOG_EVENT_VPNSERVICE     = (LOG_CATEGORY_VPNSERVICE << 16) | 41,
...
unsigned int simple_pow(unsigned int x, unsigned int y);
...
#if (defined INCLUDE_AFP)
#if (defined SECURITY_ENHANCEMENT)
extern void afp_write_shadow_passwd(char *new_passwd);
#else
extern void afp_write_shadow_passwd(void);
#endif
extern int afppasswd_backup(void);
extern int afppasswd_restore(void);
#endif
#endif
