...
{
    merlin_control_t control = {};
    control.tx_cfg.pre = pre;
    control.tx_cfg.main = main;
    control.tx_cfg.post1 = post1;
    control.tx_cfg.post2 = post2;
    control.tx_cfg.hpf = hpf;
    return merlin_callbacks.merlin_ioctl(port_to_lane[port], MERLIN_CMD_TXCFG_SET, &control);
}
...
int lport_serdes_get_tx_cfg(uint32_t port, uint16_t *pre, uint16_t *main, uint16_t *post1, uint16_t *post2, uint16_t *hpf)
{
    ...
    *pre = control.tx_cfg.pre;
    *main = control.tx_cfg.main;
    *post1 = control.tx_cfg.post1;
    *post2 = control.tx_cfg.post2;
    *hpf = control.tx_cfg.hpf;
exit:    
    return rc;
}
