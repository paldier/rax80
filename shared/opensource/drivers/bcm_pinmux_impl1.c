...
{
	bcm_set_pinmux( 6, BP_PINMUX_VAL_4);
	bcm_set_pinmux( 7, BP_PINMUX_VAL_4);
	bcm_set_pinmux( 8, BP_PINMUX_VAL_4);
	bcm_set_pinmux( 9, BP_PINMUX_VAL_4);
	GPIO->GPIODir[GPIO_NUM_TO_ARRAY_IDX(6)] &= ~GPIO_NUM_TO_MASK(6);
	GPIO->GPIODir[GPIO_NUM_TO_ARRAY_IDX(7)] &= ~GPIO_NUM_TO_MASK(7);
	GPIO->GPIODir[GPIO_NUM_TO_ARRAY_IDX(8)] &= ~GPIO_NUM_TO_MASK(8);
	GPIO->GPIODir[GPIO_NUM_TO_ARRAY_IDX(9)] &= ~GPIO_NUM_TO_MASK(9);
}
#endif
...
#if defined (_BCM94908_) /* AX6000 */
    if (!strcmp(boardIdStr, "AX6000"))
    {
        /* set hwled off */ 
