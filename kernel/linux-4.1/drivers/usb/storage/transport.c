...
#if defined(AX6000)
    //printk("%s(%d) devpath=%s usb1_pkt_cnt=%d, usb2_pkt_cnt=%d\n",__func__,__LINE__,devpath,usb1_pkt_cnt,usb2_pkt_cnt);
    if (!strncmp(devpath, "2", 1))
    {
        usb1_pkt_cnt++;
        usb1_pkt_cnt_smp++;
    }
    else if (!strncmp(devpath, "1", 1))
    {
        usb2_pkt_cnt++;
        usb2_pkt_cnt_smp++;
    }
