...
#define LOG_INDEX_FILE "/data/router-analytics/reboot_log_index"
#define LOG_FILE "/data/router-analytics/reboot_by_crash_%d.txt"

extern char *get_logbuf(void);
extern int get_logsize(void);


static int set_log_index(int index)
{
    int ret = -1;
    struct file *fp;
    mm_segment_t fs;
    ...
    fp = filp_open(LOG_INDEX_FILE, O_RDWR | O_TRUNC | O_CREAT,  S_IRUSR | S_IWUSR);
    if (!IS_ERR(fp))
    {
        fp->f_pos = 0;
        sprintf(buf, "%d", index);
...
static int get_log_index(void)
{        
    struct file *fp;
    mm_segment_t fs;
    int len;
    char buf[64];
    int strLen = sizeof(buf);
    int index;

    memset(buf, 0x00, strLen);
    fp = filp_open(LOG_INDEX_FILE, O_RDONLY, 0);
    if (!IS_ERR(fp))
    {
        fs = get_fs();
        set_fs(get_ds());
        ...
        if((len = (int) vfs_read(fp, (void *) buf, strLen,
        &fp->f_pos)) <= 0)
...
static int flash_write_debug_log(char* file_name)
{
    int ret = -1;
    struct file *fp;
    mm_segment_t fs;
    ...
    fp = filp_open(file_name, O_RDWR | O_TRUNC | O_CREAT,  S_IRUSR | S_IWUSR);
    if (!IS_ERR(fp))
    {
        /* write timestamp */
        sprintf(buf,"\n==========================Kernel crash log  %d/%d/%d %02d:%02d:%02d==============================\n", \
            (int)(1900 + tm_val.tm_year), (int)tm_val.tm_mon + 1, (int)tm_val.tm_mday, tm_val.tm_hour, tm_val.tm_min,tm_val.tm_sec);
        ...
        if((int) vfs_write(fp, (void *) log_buf, buf_len, &fp->f_pos) == buf_len)
        {       
            vfs_fsync(fp, 0);
            ret = 0;
        }
...
int flash_write_buffer(void)
{
    ...
    sprintf(log_file, LOG_FILE, index);

    flash_write_debug_log(log_file);
