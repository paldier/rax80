...
skip_rx_handler:
	...
	/* Foxconn added start pling 02/15/2017 */
	/* PPPoE throughput enhancement, port from R8500 */
	if (type == __constant_htons(ETH_P_PPP_SES)) {
#define PPP_NW_LAYER    0x3F
		if ((*((unsigned char *)skb->data + 6) <= PPP_NW_LAYER) &&
			(*((unsigned short *)((unsigned char *)skb->data + 6)) > 0)) {
			type = __constant_htons(ETH_P_PPPOE_SESS);
		}	
	}
    /* Foxconn added end pling 02/15/2017 */
