...
#else /* ! CATHY_PPTP_DL_ACC */
int blog_support_tcp_ack_mflows_g = 1;
#endif /* ! CATHY_PPTP_DL_ACC */
#else
int blog_support_tcp_ack_mflows_g = 0;
#endif
...
{
    blog_lock();
    blog_notify(CLEAR_NETIF_STATS, (void*)dev_p, (unsigned long)NULL, (unsigned long)NULL);
    blog_unlock();
 ...
 *------------------------------------------------------------------------------
 */
static void blog_tcpack_prio( Blog_t * blog_p )
{
...
#error "BLOG_TCPACK_MAX_COUNT > 15"
    ...
    if(SKBMARK_GET_FLOW_ID(blog_p->mark))
    {
        /* already classified by some other criteria */
    ...
    switch ( request )
    {
        ...
        case FLOWTRACK_PUT_STATS:
        {
            if ( likely(net_p)  )
            {
                ...
                blog_ct_put_stats(blog_p->ct_p[BLOG_CT_PLD], blog_p->nf_dir_pld,
                        stats_p);

                if(blog_p->ct_p[BLOG_CT_DEL])
                    blog_ct_put_stats(blog_p->ct_p[BLOG_CT_DEL], blog_p->nf_dir_pld,
                            stats_p);
        ...
        case FLOW_EVENT_DEACTIVATE:
        {
            if ( likely(net_p)  )
            {
                ...
                info.ct_pld_p = blog_p->ct_p[BLOG_CT_PLD];
                /*TODO check if any extra checks are neeeded to ensure dt_del_p is 
                 * a valid nf_conn*/
                info.ct_del_p = blog_p->ct_p[BLOG_CT_DEL];
