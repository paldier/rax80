...
#if defined(CONFIG_BCM_NETFILTER_REGARDLESS_DROP_NO_ACCEL_FIRST)
...
ip_conntrack_fc_resume(struct sk_buff *skb, struct nf_conn *ct, u_int8_t isv4)
{
   ...
   mark = ct->mark;
   if (ct->mark != 0)  {
       B_IQOS_SET_CTMARK(skb->fkb_mark ,mark);
   }
   return;
}
...
#endif
#if 1  // 2015/07/16 Mos, Add one by one option for portforwading in different port range
    ct->ext_port_start= 0;
#endif
