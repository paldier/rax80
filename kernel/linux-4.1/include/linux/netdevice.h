...
//#define IQOS_DUMMY_FN

typedef int (*fwdcb_t)(void *skb, struct net_device *dev);
typedef int (*br_fwdcb_t)(void *skb, struct net_device *dev); //Need to remove if TDTS use fwdcb_t instead
...
#define B_IQOS_SET_CTMARK(sfm, m) (sfm |= (m << B_IQOS_CTMARK_SHIFT))
#define B_IQOS_GET_CTMARK(sfm) 	(sfm >> B_IQOS_CTMARK_SHIFT)
...
#if defined(CONFIG_BCM_KF_BLOG) && defined(CONFIG_BLOG)
/* NOTE : To satisfy WLAN drivers backward compatibility requirement */
