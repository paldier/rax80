...
#error "NUM_CORE != 3"
#endif
static int pid2core[NUM_CORE] = {1, 0, 2};
#endif
#endif
/*
 * +-----------------------------------------------------
...
static int bcm963xx_pcie_msi_enable(struct bcm963xx_pcie_hcd *pdrv)
{
	...
	if (HCD_USE_DT_ENTRY(pdrv->core_id)) {
...
static irqreturn_t bcm963xx_pcie_errlog_isr(int irq, void *data)
{
	...
	if (int_status & CPU_INTR1_PCIE_UBUS_CPU_INTR) {
	    ...
	    while (reg_val != 0x00000000) {
	        if (reg_val & (1ul << (i))) {
	            if (i >= sizeof(bcm963xx_pcie_ubus_intr_str)/sizeof(bcm963xx_pcie_ubus_intr_str[0])) {
	                HCD_LOG("Core [%d] UBUS Intr [UnKnown_%d]\r\n", pdrv->core_id, i);
	            } else {
	                HCD_LOG("Core [%d] UBUS Intr [%s]\r\n", pdrv->core_id, bcm963xx_pcie_ubus_intr_str[i]);
	            }
	            reg_val &= (~(1ul << (i)));
	        }
	        i++;
...
static int bcm963xx_pcie_setup_regs(struct bcm963xx_pcie_hcd *pdrv)
{
	...
	struct device *dev;
	uint32 reg_data;
	int    chip_id = kerSysGetChipId();
	int    chip_rev = UtilGetChipRev();
...
static int bcm963xx_pcie_parse_dt(struct bcm963xx_pcie_hcd *pdrv)
{
	...
	if (np) {
	    ...
	    HCD_FAIL_ON_DT_ERROR("reg (core memory)", err);
...

	    {
	        ...
	        if (err == 0) {
	            pdrv->hc_cfg.apon = (dt_val) ? TRUE : FALSE;
	        }
...
static int bcm963xx_pcie_init_res(struct bcm963xx_pcie_hcd *pdrv)
{
	...
	switch (pdrv->core_id) {
	    ...
	    case 3:
	        ...
	        pres->owin.start = PCIE3_MEM_PHYS_BASE;
