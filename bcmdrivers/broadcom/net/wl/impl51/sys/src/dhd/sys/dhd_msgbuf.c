 ...
 *
 * $Id: dhd_msgbuf.c 767099 2018-08-28 09:03:48Z $
 */
...
typedef struct dhd_prot {
	...
	dhd_mb_ring_2_t	db1_2_ring_fn;	/* HWA iDMA specific, dongle doesn't handle this signal */
	...
	dhd_dma_buf_t	host_mem_buf;	/* Extension of dongle memory in host */
...
#if (defined(BCM_ROUTER_DHD) && defined(HNDCTF)) && !defined(BCM_DHD_RUNNER)
...
#if defined(BCM_ROUTER_DHD) && defined(BCM_GMAC3)
	/* Force cacheline alignment for optimum ACP performance on CA9 */
#if defined(CONFIG_ARM_L1_CACHE_SHIFT)
	dma_align = (0x1 << CONFIG_ARM_L1_CACHE_SHIFT);
#else
	dma_align = 64;
#endif // endif
...
dhd_bcm_buzzz_attach(dhd_pub_t *dhd)
{
	...
	{
		if (dhd_dma_buf_alloc(dhd, dhd->bcm_buzzz_dma_buf + seg, dma_buf_len))
		{
		...
		/*
		 * already memset to 0, flushed in dhd_dma_buf_alloc->dhd_dma_buf_reset
		 * memset(dhd->bcm_buzzz_va[seg], 0, dma_buf_len);
		 * OSL_CACHE_INV(dhd->bcm_buzzz_va[seg], dma_buf_len);
		 */
...
dhd_bcm_buzzz_detach(dhd_pub_t *dhd)
{
	...
	for (seg = 0; seg < BCM_BUZZZ_SEGMENTS; seg++) {
		...
		dhd_dma_buf_free(dhd, dhd->bcm_buzzz_dma_buf + seg);
	}
...
dhd_prot_attach(dhd_pub_t *dhd)
{
	...
	/* Host memory used as an extension by dongle */
	{

		/* Sections are padded to 4096 Bytes - a DHD page unit */
		const uint32 rounding = 4096;
		...
		DHD_INFO(("%s: Host Extension Memory DMA Buffer len %u rounding %u\n",
			__FUNCTION__, dma_buf_len, rounding));

		if (dhd_dma_buf_alloc(dhd, &prot->host_mem_buf, dma_buf_len)) {
			goto fail;
		}
	}
	...
	uint32 flags)
{
	...
	switch (txpost_format)
	{
		...
		default:
			DHD_ERROR(("Invalid txpost_format %u\n", rxpost_format)); ASSERT(0);
	...
	switch (rxpost_format)
	{
		...
		default:
			DHD_ERROR(("Invalid rxpost_format %u\n", rxpost_format)); ASSERT(0);
			return BCME_BADARG;
	}

	max = H2DRING_RXPOST_MAX_ITEM >> misc;

	DHD_ERROR(("H2D RxPost %u: format %u = type %u size %2u max %4u shift %u\n",
	...
	if (dhd_prot_ring_alloc(dhdp, &prot->h2dring_rxp_subn, "h2drxp", ringid,
		...
		DHD_ERROR(("H2D RxPost Ring Alloc failed\n")); ASSERT(0);
		return BCME_NOMEM;
	}
	...
	switch (txcpln_format)
	{
		...
		default:
			DHD_ERROR(("Invalid txcpln_format %u\n", txcpln_format)); ASSERT(0);
			return BCME_BADARG;
	}

	max = D2HRING_TXCMPLT_MAX_ITEM;

	DHD_ERROR(("H2D TxCpln %u: format %u = type %u size %2u max %4u\n",
		ringid, txcpln_format, type, size, max));

	if (dhd_prot_ring_alloc(dhdp, &prot->d2hring_tx_cpln, "d2htxcpl", ringid,
	        type, size, max, misc) != BCME_OK) {
		DHD_ERROR(("D2H Tx Completion Ring Alloc failed\n")); ASSERT(0);
		return BCME_NOMEM;
	}
	...
	switch (rxcpln_format)
	{
		...
		default:
			DHD_ERROR(("Invalid rxcpln_format %u\n", rxcpln_format)); ASSERT(0);
			return BCME_BADARG;
	}

	max = D2HRING_RXCMPLT_MAX_ITEM;

	DHD_ERROR(("H2D RxCpln %u: format %u = type %u size %2u max %4u offset %u\n",
		ringid, rxcpln_format, type, size, max, misc));

	/* Allocate pool of msgbuf_ring_t objects for d2hring_rx_cpln */
	prot->d2hring_rx_cpln = (msgbuf_ring_t *)MALLOCZ(prot->osh,
		(max_rxcpln_rings * sizeof(msgbuf_ring_t)));

	if (prot->d2hring_rx_cpln == NULL) {
		DHD_ERROR(("%s: d2hring_rx_cpln for %d max_rxcpln_rings, alloc failure\n",
			__FUNCTION__, max_rxcpln_rings));
		return BCME_NOMEM;
	}

	for (i = 0; i < max_rxcpln_rings; i++) {
		if (dhd_prot_ring_alloc(dhdp, DHD_RING_IN_D2HRING_RX_CPLN(prot, i), "d2hrxcpl",
			(ringid + i), type, size, max, misc) != BCME_OK) {
			DHD_ERROR(("D2H Rx Completion Ring Alloc failed\n")); ASSERT(0);
			return BCME_NOMEM;
		}
	}
...
dhd_prot_init(dhd_pub_t *dhd)
{
	...
	PHYSADDRLOSET(prot->host_mem_buf.pa,
		PHYSADDRLO(prot->host_mem_buf.pa) + ACP_WIN_SIZE);
#endif /* BCM_ROUTER_DHD && BCM_GMAC3 */

	dhd_base_addr_htolpa(&base_addr, prot->host_mem_buf.pa);
	dhd_bus_cmn_writeshared(dhd->bus, &base_addr, HOST_MEM_BUF_ADDR, 0);
	dhd_bus_cmn_writeshared(dhd->bus, &prot->host_mem_buf.len, HOST_MEM_BUF_LEN, 0);
	...
	if (IFRM_ENAB(dhd)) {
		...
		dhd_bus_cmn_writeshared(dhd->bus, &base_addr, H2D_IFRM_INDX_WR_BUF, 0);
	}
...
dhd_prot_detach(dhd_pub_t *dhd)
{
	...
	if (prot) {
		...
		dhd_dma_buf_free(dhd, &prot->host_mem_buf);
...
#else /* ! NDIS */
		ASSERT(!dhd->prot->pktid_map_handle && !dhd->prot->pktid_map_handle_ioctl);
#endif /* ! NDIS */
...
dhd_prot_reset(dhd_pub_t *dhd)
{
	...
	dhd_dma_buf_reset(dhd, &prot->host_mem_buf);
...
dhd_sync_with_dongle(dhd_pub_t *dhd)
{
	...
	dhd_macdbg_upd_corerev(dhd, revinfo.corerev);
	...
	if (PKTBUF) {
...
#endif /* BCM_ROUTER_DHD && BCM_GMAC3 */
		{
			if (SECURE_DMA_ENAB(dhd->osh))  {
				...
				SECURE_DMA_UNMAP(dhd->osh, paddr, (uint) len, DMA_TX, 0, dmah,
					secdma, 0);
#else
...
#ifdef BCMDMA64OSL
				PHYSADDRTOULONG(pa, paddr);
				SECURE_DMA_UNMAP(dhd->osh, paddr, (uint) len, DMA_RX, 0, dmah,
					secdma, 0);
#else
...
dhd_prot_rxbuf_post(dhd_pub_t *dhd, bool use_rsv_pktid)
{
...

	{
		...
		for (i = 0, curpkt = &rxp_pktpool[0]; i < pkt_count; i++, curpkt++)
		{
			...
			/* 2KBytes packet total buffer len is inclusive of any headroom */
			...
			PKTPULL(dhd->osh, curpkt->ptr, BCMEXTRAHDROOM);
#endif /* BCM_ROUTER_DHD */
#ifdef WL_MONITOR
		if (dhd_monitor_enabled(dhd, 0)) {
			PKTPULL(dhd->osh, curpkt->ptr, sizeof(wl_radiotap_vht_t));
		}
#endif /* WL_MONITOR */
			...
			if (SECURE_DMA_ENAB(dhd->osh)) {
				DHD_GENERAL_LOCK(dhd, flags);
#ifdef BCMDMA64OSL
				pa = SECURE_DMA_MAP(dhd->osh, pkt_buf_va, pkt_buf_len, DMA_RX,
						curpkt->ptr, 0, rxp_ring->dma_buf.secdma, 0,
						CMA_RXBUF_POST);
				ULONGTOPHYSADDR(pa, curpkt->buf_pa);
#else
				curpkt->buf_pa =
...
#endif // endif
			curpkt->pktid =
				DHD_NATIVE_TO_PKTID(dhd, dhd->prot->pktid_map_handle,
		...
		for (i = 0, curpkt = &rxp_pktpool[0]; i < pkt_count; i++, curpkt++)
		{
			...
			if (PHYSADDRISZERO(curpkt->buf_pa)) {
				...
				if (SECURE_DMA_ENAB(dhd->osh)) {
					DHD_GENERAL_LOCK(dhd, flags);
#ifdef BCMDMA64OSL
					PHYSADDRTOULONG(curpkt->buf_pa, paddr);
					SECURE_DMA_UNMAP(dhd->osh, paddr, pktlen, DMA_RX,
...
dhd_prot_rxbuf_post_lb(dhd_pub_t *dhd, bool use_rsv_pktid)
{
	...
	for (i = 0; i < alloced; i++) {
		...
		if (SECURE_DMA_ENAB(dhd->osh)) {
			DHD_GENERAL_LOCK(dhd, flags);
#ifdef BCMDMA64OSL
			paddr = SECURE_DMA_MAP(dhd->osh, PKTDATA(dhd->osh, p), pktlen,
...
#ifdef BCMDMA64OSL
				PHYSADDRTOULONG(pa, paddr);
				SECURE_DMA_UNMAP(dhd->osh, paddr, pktlen, DMA_RX, 0, DHD_DMAH_NULL,
...
#ifdef BCMDMA64OSL
					PHYSADDRTOULONG(pa, paddr);
					SECURE_DMA_UNMAP(dhd->osh, paddr,
...
#if (defined(BCM_ROUTER_DHD) && defined(HNDCTF))
		if (!dhd_rx_pkt_integrity(dhd, ifidx)) {
			DHD_ERROR(("%s: recv pkt from a removed ifidx %d !\n",
			          __FUNCTION__, ifidx));
			PKTFREE(dhd->osh, pkt, FALSE); /* no receiving interface, free */
			continue;
		}
#endif /* BCM_ROUTER_DHD && HNDCTF */
...
void dhd_prot_stop(dhd_pub_t *dhd)
{
	...
	if (dhd->prot) {
		if (dhd->prot->pktid_map_handle) {
			DHD_NATIVE_TO_PKTID_FINI(dhd, dhd->prot->pktid_map_handle);
			dhd->prot->pktid_map_handle = NULL;
		} else {
			DHD_ERROR(("%s: pktid_map_handle already de-initted\n", __FUNCTION__));
		}

#if defined(DHD_PCIE_PKTID)
		if (dhd->prot->pktid_map_handle_ioctl) {
			DHD_NATIVE_TO_PKTID_FINI_IOCTL(dhd, dhd->prot->pktid_map_handle_ioctl);
			dhd->prot->pktid_map_handle_ioctl = NULL;
		} else {
			DHD_ERROR(("%s: pktid_map_handle_ioctl already de-initted\n",
				__FUNCTION__));
		}
#endif /* DHD_PCIE_PKTID */
	}
...
#if defined(DHD_PCIE_PKTID)
	{
		uint16 pktid_avail = (uint16)DHD_PKTID_AVAIL(dhd->prot->pktid_map_handle);
...
retry_ring_space:
	...
	for (nitems = 0; nitems < alloced; nitems++)
	{
		...
		if (pkt == NULL) { /* flow_ring's queue is empty .... hmmm */
			DHD_ERROR(("%s: NULL pkt, queue_avail<%u> alloced<%u>\n",
			           __FUNCTION__, queue_avail, alloced));
			ASSERT(pkt != NULL);
			try_once = ~0;
		...
		if (PKTISFWDERBUF(dhd->osh, pkt)) {

			int dataoff = DHD_PKT_GET_DATAOFF(pkt); /* fixed offset of .3 pkt */
			pkthdr = pktdata + dataoff; /* start of 802.3 pkthdr */

			/* Copy entire ethernet header: (32bit x 3) + (16bit x 1) */
			ehcopy32(pkthdr, txp_wi_pkthdr);

			pktlen -= (uint16)(dataoff + ETHER_HDR_LEN + ETHER_CRC_LEN);

			/* Dongle requests DHD to insert LLCSNAP */
			if (DHDHDR_SUPPORT(dhd)) {
				uint8 *d = pkthdr - DOT11_LLC_SNAP_HDR_LEN;

				/* Ensure space for LLCSNAP */
				ASSERT(dataoff >= DOT11_LLC_SNAP_HDR_LEN);

				/* Insert LLCSNAP */
				dhd_prot_insert_llcsnap(pkthdr, d,
					DOT11_LLC_SNAP_HDR_LEN + pktlen,
					((struct ether_header*)pkthdr)->ether_type, TRUE);

				/* Cache flush one line */
				OSL_CACHE_FLUSH_LINE(d);
			}

			/* Location of Ethernet payload */
			pktdata += (dataoff + ETHER_HDR_LEN);

			/* DDR alias to noacp region */
			pa = virt_to_phys_noacp(pktdata);
		}
		else
#endif /* !BCM_NO_WOFA */
		{
		...
		pktid = DHD_NATIVE_TO_PKTID(dhd, dhd->prot->pktid_map_handle, pkt,
		...
		switch (ring->item_type)
		{
			...
			case MSGBUF_WI_CWI32:
			{
				...
				// cwi32:: info, flowid_override for future use.
#ifdef BCMHWA /* Remove it when we don't support 43684a0 HWA anymore */
				if (prot->hwa_rev == 128)
					cwi32->flowid_override = htol16(flowid & 0xFFF);
#endif // endif
				break;
			}
			...
			case MSGBUF_WI_CWI64:
			{
				...
				// cwi64:: info, flowid_override for future use
#ifdef BCMHWA /* Remove it when we don't support 43684a0 HWA anymore */
				if (prot->hwa_rev == 128)
					cwi64->flowid_override = htol16(flowid & 0xFFF);
#endif // endif
				break;
			}
...
done_queue_processing:
	...
	if (nitems) { /* at least one pkt was posted */
		/* update ring's WR index and ring doorbell to dongle */
		dhd_prot_ring_write_complete(dhd, ring, (void*)txp_wi_start, nitems);
...
dhd_prot_txdata(dhd_pub_t *dhd, void *PKTBUF, uint8 ifidx)
{
	...
	if (SECURE_DMA_ENAB(dhd->osh)) {
#ifdef BCMDMA64OSL
		paddr = SECURE_DMA_MAP(dhd->osh, pktdata, pktlen,
			DMA_TX, PKTBUF, 0, ring->dma_buf.secdma, 0, CMA_TXBUF_POST);
		ULONGTOPHYSADDR(paddr, pa);
#else
...
#else
	/* update ring's WR index and ring doorbell to dongle */
	dhd_prot_ring_write_complete(dhd, ring, txp_wi_start, 1);
#endif // endif
...
#else
	bcm_bprintf(b, "DHD Router: 3GMAC-WOFA forwarding mode\n");
#endif /* BCM_NO_WOFA */
#else
...
dhd_prot_flowrings_pool_alloc(dhd_pub_t *dhd)
{
	...
	FOREACH_RING_IN_FLOWRINGS_POOL(prot, ring, flowid) {
		...
		if (IDMA_ACTIVE(dhd)) {
			ring->idma_index = dhd_idma_flowmgr_ringid_2_index(
				dhd->flowid_allocator, ring->id, flowid);
			DHD_INFO(("%s: Ring \"%s\" ringid %d flowid %d idma_index %d\n",
	...
	uint16 nitems)
{
	...
	if (!HWA_RXPOST_ACTIVE(ring)) {
		...
		} else if (IFRM_ACTIVE(dhd) && DHD_IS_FLOWRING(ring->id)) {
			/* iFRM use other DMA index buffer, h2d_ifrm_indx_wr_buf */
			dhd_prot_dma_indx_set(dhd, ring->wr, H2D_IFRM_INDX_WR_UPD, ring->id);
		} else {
	...
	if (HWA_RXPOST_ACTIVE(ring)) {
		...
		db_index = ((DMA_TYPE_IDMA << DMA_TYPE_SHIFT) |
			(DHD_H2D_FRM_FLOW_RING_OFFSET(ring->id) /
			dhd->bus->ifrm_index_num_per_group));
		prot->mb_2_ring_fn(dhd->bus, db_index, TRUE);
	} else {
...
dhd_prot_dma_indx_init(dhd_pub_t *dhd, uint32 rw_index_sz, uint8 type, uint32 length)
{
	...
	switch (type) {
		case H2D_DMA_INDX_WR_BUF:
			...
			DHD_ERROR(("H2D DMA WR INDX : %p array size %d = %d * %d\n",
				dma_buf->va, dma_buf->len, rw_index_sz, length));
			break;
		...
		case H2D_DMA_INDX_RD_BUF:
			...
			DHD_ERROR(("H2D DMA RD INDX : %p array size %d = %d * %d\n",
				dma_buf->va, dma_buf->len, rw_index_sz, length));
			break;
		...
		case D2H_DMA_INDX_WR_BUF:
			...
			DHD_ERROR(("D2H DMA WR INDX : %p array size %d = %d * %d\n",
				dma_buf->va, dma_buf->len, rw_index_sz, length));
			break;
		...
		case D2H_DMA_INDX_RD_BUF:
			...
			DHD_ERROR(("D2H DMA RD INDX : %p array size %d = %d * %d\n",
				dma_buf->va, dma_buf->len, rw_index_sz, length));
			break;
		...
		case H2D_IFRM_INDX_WR_BUF:
			...
			if (dhd_prot_dma_indx_alloc(dhd, type, dma_buf, bufsz))
				goto ret_no_mem;
			DHD_ERROR(("H2D IFRM WR INDX : %p array size %d = %d * %d\n",
				dma_buf->va, dma_buf->len, rw_index_sz, length));
			break;
...
dhd_prot_flow_ring_create(dhd_pub_t *dhd, flow_ring_node_t *flow_ring_node)
{
	...
	DHD_INFO(("%s: Send Flow Create Req flow ID %d for peer " MACDBG
		" prio %d ifindex %d\n", __FUNCTION__, flow_ring_node->flowid,
	...
	} else if (IFRM_ACTIVE(dhd) && DHD_IS_FLOWRING(flow_ring->id)) {
		dhd_prot_dma_indx_set(dhd, flow_ring->wr,
			H2D_IFRM_INDX_WR_UPD, flow_ring->id);
	} else {
		dhd_bus_cmn_writeshared(dhd->bus, &(flow_ring->wr),
			RING_WR_UPD, flow_ring->id);
	}

	/* Raise h2d interrupt individually, because the iDMA separate H2D Ctrl and
	 * flow ring wr index into different Set
	 */
	if (IDMA_ACTIVE(dhd)) {
		uint32 db_index;
		db_index = ((DMA_TYPE_IDMA << DMA_TYPE_SHIFT) | flow_ring->idma_index);
		prot->mb_2_ring_fn(dhd->bus, db_index, TRUE);
	}
	...
	struct bcmstrbuf *strbuf, const char * fmt)
{
	...
	/* H2D FLOWRING  */
		...
		if (IFRM_ACTIVE(dhd))
			host_wr = dhd_prot_dma_indx_get(dhd, H2D_IFRM_INDX_WR_UPD, flow_ring->id);
		else if (DMA_INDX_ENAB(dhd->dma_h2d_ring_upd_support))
			host_wr = dhd_prot_dma_indx_get(dhd, H2D_DMA_INDX_WR_UPD, flow_ring->id);
		else
			host_wr = flow_ring->wr;
	/* D2H RING */
...
dhd_prot_flow_ring_delete_response_process(dhd_pub_t *dhd, void *msg)
{
	...
	DHD_ERROR(("%s: Flow Delete Response status = %d \n", __FUNCTION__,
...
dhd_prot_runner_txstatus_process(dhd_pub_t *dhd, void* msg)
{
	...
	if (dhd->prot->txpost_item_type == MSGBUF_WI_WI64) {
	    flow_ring_node_t *flow_ring_node;
	    host_txbuf_cmpl_t * txstatus;
	    txstatus = (host_txbuf_cmpl_t *)msg;
	    ...
	    if (DHD_FLOWRING_RNR_OFFL(flow_ring_node)) {
	        ...
	        if (DHD_RNR_OFFL_NONACCPKT_TXSTS(dhd)) {
	            ...
	            DHD_ERROR(("%s: Unexpected TX_STATUS from Runner on flowring <%d>\r\n",
	                __FUNCTION__, flow_ring_node->flowid));
	            ASSERT(FALSE);
	            DHD_PERIM_UNLOCK_ALL(dhd->fwder_unit % FWDER_MAX_UNIT);
	            return;
	            ...
	            if (pkt != NULL) {
	                ...
	                pa = virt_to_phys(PKTDATA(dhd->osh, pkt));
...
void
dhd_prot_set_hwa_rev(dhd_pub_t *dhd, uint32 rev)
{
	dhd_prot_t *prot = dhd->prot;
