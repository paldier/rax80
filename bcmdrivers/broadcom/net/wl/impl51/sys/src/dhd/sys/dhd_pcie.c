 ...
 *
 * $Id: dhd_pcie.c 766880 2018-08-20 01:56:26Z $
 */
...
static int dhdpcie_bus_membytes(dhd_bus_t *bus, bool write,
...
static uint8 dhdpcie_bus_rtcm8(dhd_bus_t *bus, uint32 daddr32);
static void dhdpcie_bus_wtcm8(dhd_bus_t *bus, uint32 daddr32, uint8 data);
static void dhdpcie_bus_wtcm16(dhd_bus_t *bus, uint32 daddr32, uint16 data);
uint16 dhdpcie_bus_rtcm16(dhd_bus_t *bus, uint32 daddr32);
static void dhdpcie_bus_wtcm32(dhd_bus_t *bus, uint32 daddr32, uint32 data);
uint32 dhdpcie_bus_rtcm32(dhd_bus_t *bus, uint32 daddr32);
#ifndef BCMQT
static void dhdpcie_bus_wtcm64(dhd_bus_t *bus, uint32 daddr32, uint64 data);
static uint64 dhdpcie_bus_rtcm64(dhd_bus_t *bus, uint32 daddr32);
#endif /* !BCMQT */
...
dhdpcie_dongle_attach(dhd_bus_t *bus)
{
	...
	if (BCM43684_CHIP(bus->sih->chip) && CHIPREV(bus->sih->chiprev) <= 1) {
		int idx = si_coreidx(bus->sih);
		if (si_setcore(bus->sih, ARMCA7_CORE_ID, 0)) {
			si_core_reset(bus->sih, SICF_CPUHALT, SICF_CPUHALT);
			si_setcoreidx(bus->sih, idx);
		}
	}
	...
	bus->ifrm_enabled = FALSE;
	DHD_ERROR(("%s: Runner: iFRM force to disable\n", __FUNCTION__));
#else
	bus->idma_enabled = TRUE;
...
#define HT4365_REQ	((1 << RES4365_HT_AVAIL) | (1 << RES4365_MACPHY_CLK_AVAIL))

/* This function is to switch Backplane clock from HT to ALP and vice versa.
 * Pass bp_on_ht as TRUE to switch to HT clock
 *	bp_on_ht as FALSE to switch to ALP clock
 */
static void
dhdpcie_bus_switch_ht_alp(dhd_bus_t *bus, bool bp_on_ht)
{
	uint32 pmu_idx = si_findcoreidx(bus->sih, PMU_CORE_ID, 0);

	DHD_ERROR(("%s: BP is currently on %s clock, force BP on %s clock\n", __FUNCTION__,
		(si_corereg(bus->sih, SI_CC_IDX, OFFSETOF(chipcregs_t, clk_ctl_st), 0, 0)
		& CCS_BP_ON_HT) ? "HT" : "ALP", bp_on_ht ? "HT" : "ALP"));

	if (bp_on_ht) {
		/* To force BP on HT clock */
		si_corereg(bus->sih, pmu_idx, PMU_MIN_RES_MASK, HT4365_REQ, HT4365_REQ);
		OSL_DELAY(100);
		si_corereg(bus->sih, pmu_idx, PMU_MAX_RES_MASK, HT4365_REQ, HT4365_REQ);
		OSL_DELAY(100);

		si_corereg(bus->sih, SI_CC_IDX, OFFSETOF(chipcregs_t, clk_ctl_st), CCS_FORCEHT,
			CCS_FORCEHT);
		SPINWAIT(((si_corereg(bus->sih, SI_CC_IDX, OFFSETOF(chipcregs_t, clk_ctl_st), 0, 0)
			& CCS_BP_ON_HT) == 0), PMU_MAX_TRANSITION_DLY);
		ASSERT(si_corereg(bus->sih, SI_CC_IDX, OFFSETOF(chipcregs_t, clk_ctl_st), 0, 0)
			& CCS_BP_ON_HT);
	} else {
		/* To force BP on ALP clock */
		si_corereg(bus->sih, pmu_idx, PMU_MIN_RES_MASK, HT4365_REQ, 0);
		OSL_DELAY(100);
		si_corereg(bus->sih, pmu_idx, PMU_MAX_RES_MASK, HT4365_REQ, 0);
		OSL_DELAY(100);

		SPINWAIT(((si_corereg(bus->sih, SI_CC_IDX, OFFSETOF(chipcregs_t, clk_ctl_st), 0, 0)
			& CCS_BP_ON_ALP) == 0), PMU_MAX_TRANSITION_DLY);
		ASSERT(si_corereg(bus->sih, SI_CC_IDX, OFFSETOF(chipcregs_t, clk_ctl_st), 0, 0)
			& CCS_BP_ON_ALP);
	}

	DHD_ERROR(("%s: BP is on %s clock now\n", __FUNCTION__,
		(si_corereg(bus->sih, SI_CC_IDX, OFFSETOF(chipcregs_t, clk_ctl_st), 0, 0)
		& CCS_BP_ON_HT) ? "HT" : "ALP"));
}
...
dhdpcie_bus_watchdog_reset(dhd_bus_t *bus)
{
	...
	if (BCM4365_CHIP(chip_id)) {
	    dhdpcie_bus_switch_ht_alp(bus, FALSE);
	}
	...
	if (BCM4365_CHIP(chip_id)) {
	    ...
	    dhdpcie_bus_switch_ht_alp(bus, TRUE);
	}
}
...
bool dhd_bus_watchdog(dhd_pub_t *dhd)
{
	...
	if (dhd->dhd_console_ms != 0) {
		...
		} else if (dhd->dongle_trap_occured == TRUE) {
			/* obtain console for last time */
...
dhdpcie_download_code_file(struct dhd_bus *bus, char *pfw_path)
{
	...
	while ((len = dhd_os_get_image_block((char*)memptr, MEMBLOCK, imgbuf))) {
		...
		bcmerror = dhdpcie_bus_membytes(bus, TRUE, daddr32, (uint8 *)memptr, len);
		if (bcmerror) {
			DHD_ERROR(("%s: error %d on writing %d membytes at 0x%08x\n",
				__FUNCTION__, bcmerror, MEMBLOCK, daddr32));
			goto err;
		}
		daddr32 += MEMBLOCK;
	}
...
#if defined(DLIMAGE_43602a1) || defined(DLIMAGE_43602a3)
...
#define CHIPIDSTR_43602         "43602"
#endif /* DLIMAGE_43602a1 || DLIMAGE_43602a3 */
...
#endif /* DLIMAGE_4366c0 */

#if defined(DLIMAGE_43684a0) || defined(DLIMAGE_43684b0)
#define CHIPID_43684            BCM43684_CHIP_ID
#define CHIP_43684_CHIPREV_A0   0
#define CHIP_43684_CHIPREV_A1   1
#define CHIP_43684_CHIPREV_B0   2
#define CHIP_43684_CHIPREV_B1   3
#define CHIP_43684_PKG_OPT      0
#define CHIPIDSTR_43684         "43684"
#endif /* DLIMAGE_43684a0 || DLIMAGE_43684b0 */
...
enum chip_image_rev
{
	...
	CHIP_43602_A3_CHIP_IMAGE,
#endif // endif
#ifdef DLIMAGE_4366c0
	CHIP_4366_C0_CHIP_IMAGE,
#endif // endif
#ifdef DLIMAGE_43684a0
	CHIP_43684_A0_CHIP_IMAGE,
#endif // endif
...
#endif // endif
#ifdef DLIMAGE_43602a3
	{CHIPID_43602, CHIP_43602_CHIPREV_A3, CHIP_43602_PKG_OPT, CHIP_43602_A3_CHIP_IMAGE,
	CHIPIDSTR_43602, CHIPREVSTR_A3},
	{CHIPID_43462, CHIP_43602_CHIPREV_A3, CHIP_43602_PKG_OPT, CHIP_43602_A3_CHIP_IMAGE,
	CHIPIDSTR_43602, CHIPREVSTR_A3},
	{CHIPID_43522, CHIP_43602_CHIPREV_A3, CHIP_43602_PKG_OPT, CHIP_43602_A3_CHIP_IMAGE,
	CHIPIDSTR_43602, CHIPREVSTR_A3},
#endif // endif
#ifdef DLIMAGE_4366c0
	/* Unprogrammed */
...
#endif /* DLIMAGE_4366c0 */
#ifdef DLIMAGE_43684a0
	{CHIPID_43684, CHIP_43684_CHIPREV_A0, CHIP_43684_PKG_OPT, CHIP_43684_A0_CHIP_IMAGE,
	CHIPIDSTR_43684, CHIPREVSTR_A0},
	/* 43684A1 uses 43684A0 image */
	{CHIPID_43684, CHIP_43684_CHIPREV_A1, CHIP_43684_PKG_OPT, CHIP_43684_A0_CHIP_IMAGE,
	CHIPIDSTR_43684, CHIPREVSTR_A0},
#endif // endif
#ifdef DLIMAGE_43684b0
...
#endif // endif
#ifdef DLIMAGE_43684a0
	{dlarray_43684a0, sizeof(dlarray_43684a0), dlimagename_43684a0,
	dlimagever_43684a0, dlimagedate_43684a0},
#endif // endif
#ifdef DLIMAGE_43684b0
...
static int select_chipidverstr(
	struct dhd_bus *bus, char *idverstr) {
	int res = BCME_OK;
	uint32 chipid, chiprev, chippkg_opt;
	...
	if (chipid != CHIPID_NONE) {
		if (idverstr) {
			strncat(idverstr, p_img_idx->chipidname, strlen(p_img_idx->chipidname));
			strncat(idverstr, p_img_idx->chiprevname, strlen(p_img_idx->chiprevname));
		}
	} else {
		DHD_ERROR(("####################################################################\n"
			"# %s: DHD not supported for chipid = 0x%x"
			"  chiprev = %d  chippkg = %d\n"
			"####################################################################\n",
			__FUNCTION__, bus->sih->chip, bus->sih->chiprev, bus->sih->chippkg));
...
concate_extfdimg_name(dhd_bus_t *bus, char *fw_path)
{
	...
	if (select_chipidverstr(bus, chipidver) == BCME_OK) {
		if (fw_path[strlen(fw_path)-1] != '/') {
			strncat(fw_path, "/", strlen("/"));
		}
		strncat(fw_path, chipidver, strlen(chipidver));
		strncat(fw_path, "/rtecdc.bin", strlen("/rtecdc.bin"));
	}
	...
	/* Upload and compare the downloaded code */
	{
		...
		while (uploded_len  < dlarray_size) {
			...
			if (remaining_len >= MEMBLOCK)
				...
				len = remaining_len;
			bcmerror = dhdpcie_bus_membytes(bus, FALSE, daddr32,
...
dhdpcie_bus_readconsole(dhd_bus_t *bus)
{
	...
	if ((rv = dhdpcie_bus_membytes(bus, FALSE, daddr32, (uint8 *)&c->log, sizeof(c->log))) < 0)
		return rv;
	...
	*/
	...
	if (idx < c->last) {
		...
		if ((rv = dhdpcie_bus_membytes(bus, FALSE,
				(ulong)daddr32 + (ulong)c->last, c->buf, readlen)) < 0) {
			DHD_ERROR(("conlog: read error[1] ! \n"));
			return rv;
		}
		/* from beginning of buffer to write ptr */
		if ((rv = dhdpcie_bus_membytes(bus, FALSE,
				(ulong)daddr32, (uint8 *)((ulong)c->buf + (ulong)readlen),
				idx)) < 0) {
...
dhdpcie_checkdied(dhd_bus_t *bus, char *data, uint size)
{
	...
	int rv = BCME_OK; /* rv used in console log upload and not returned */
	uint msize = 512;
	char *mbuffer = NULL;
	 ...
	 * Do not need to read pcie_ipc_rings
	 */
	bcmerror = dhdpcie_bus_read_pcie_ipc(bus, NULL, pcie_ipc, NULL);
	...
	if ((pcie_ipc->flags & (PCIE_IPC_FLAGS_ASSERT | PCIE_IPC_FLAGS_TRAP)) == 0) {
		...
		if (pcie_ipc->flags & PCIE_IPC_FLAGS_ASSERT) {
			...
			if (pcie_ipc->assert_exp_daddr32 != 0) {
				...
				if (bcmerror < 0) {
					DHD_ERROR(("%s: read expr daddr32 0x%08x FAILURE %d\n",
						__FUNCTION__, pcie_ipc->assert_exp_daddr32,
						bcmerror));
					goto done;
				}
			...
			if (pcie_ipc->assert_file_daddr32 != 0) {
				...
				if (bcmerror < 0) {
					DHD_ERROR(("%s: read file daddr32 0x%08x FAILURE %d\n",
						__FUNCTION__, pcie_ipc->assert_file_daddr32,
						bcmerror));
					goto done;
				}
		...
		if (pcie_ipc->flags & PCIE_IPC_FLAGS_TRAP) {
			...
			if (bcmerror < 0) {
				DHD_ERROR(("%s: read trap daddr32 0x%08x FAILURE %d\n",
					__FUNCTION__, pcie_ipc->trap_daddr32, bcmerror));
				goto done;
			}
		...
		if ((rv = dhdpcie_bus_membytes(bus, FALSE, daddr32,
 ...
 */
static int
dhdpcie_bus_membytes(dhd_bus_t *bus, bool write, daddr32_t daddr32, uint8 *data, uint size)
{
	uint dsize;
	...
	if (write) {
		while (size) {
			...
			if (size >= sizeof(uint32)) {
				dhdpcie_bus_wtcm32(bus, daddr32, (uint32) HTOL32(*(uint32*)data));
#else
...
#endif /* CONFIG_64BIT */
				1) {
				dhdpcie_bus_wtcm64(bus, daddr32, *((uint64 *)data));
#endif /* BCMQT */
			} else {
				dsize = sizeof(uint8);
				dhdpcie_bus_wtcm8(bus, daddr32, *data);
			}
...
#ifdef BCMQT
			if (size >= sizeof(uint32)) {
				*(uint32*)data = LTOH32(dhdpcie_bus_rtcm32(bus, daddr32));
#else
			if (size >= sizeof(uint64) && little_endian &&
#ifdef CONFIG_64BIT
				!(daddr32 % 8) &&
#endif /* CONFIG_64BIT */
				1) {
				*(uint64 *)data = dhdpcie_bus_rtcm64(bus, daddr32);
#endif /* BCMQT */
			} else {
				dsize = sizeof(uint8);
				*data = dhdpcie_bus_rtcm8(bus, daddr32);
			}
	...
	if (DHD_FLOWRING_RNR_OFFL(flow_ring_node))
	{
		...
		while ((txp = dhd_flow_queue_dequeue(bus->dhd, queue)) != NULL) {
			...
			if (PKTLEN(bus->dhd->osh, txp) < (ETHER_MIN_LEN - ETHER_CRC_LEN)) {
				uint8 *pad_ptr = (uint8 *) PKTDATA(bus->dhd->osh, txp) +
				        PKTLEN(bus->dhd->osh, txp);
				int pad_size = (ETHER_MIN_LEN - ETHER_CRC_LEN) -
				        PKTLEN(bus->dhd->osh, txp);
			...
			/* if this is WMF UCAST packets and not fully cloned,
			 *  will do full clone here and change header
			 */
			if (IS_FKB_WMF_UCAST(txp)) {
				if (FKB_IS_FKBPOOL(txp)) {
...
int dhd_bus_console_in(dhd_pub_t *dhd, uchar *msg, uint msglen)
{
	...
	val = htol32(0);
	if ((rv = dhdpcie_bus_membytes(bus, TRUE, daddr32, (uint8 *)&val, sizeof(val))) < 0)
		goto done;

	/* Write message into cbuf */
	daddr32 = bus->console_daddr32 + OFFSETOF(hnd_cons_t, cbuf);
	if ((rv = dhdpcie_bus_membytes(bus, TRUE, daddr32, (uint8 *)msg, msglen)) < 0)
		goto done;
...
void
dhdpcie_bus_wtcm8(dhd_bus_t *bus, uint32 daddr32, uint8 data)
{
	*(volatile uint8 *)(bus->tcm + daddr32) = (uint8)data;
}

uint8
dhdpcie_bus_rtcm8(dhd_bus_t *bus, uint32 daddr32)
{
	volatile uint8 data;
...
void
dhdpcie_bus_wtcm32(dhd_bus_t *bus, uint32 daddr32, uint32 data)
{
	*(volatile uint32 *)(bus->tcm + daddr32) = (uint32)data;
}

void
dhdpcie_bus_wtcm16(dhd_bus_t *bus, uint32 daddr32, uint16 data)
{
...
#ifndef BCMQT
void
dhdpcie_bus_wtcm64(dhd_bus_t *bus, uint32 daddr32, uint64 data)
{
...
uint16
dhdpcie_bus_rtcm16(dhd_bus_t *bus, uint32 daddr32)
{
...
uint32
dhdpcie_bus_rtcm32(dhd_bus_t *bus, uint32 daddr32)
{
...
#ifndef BCMQT
uint64
dhdpcie_bus_rtcm64(dhd_bus_t *bus, uint32 daddr32)
{
...
dhd_bus_cmn_writeshared(dhd_bus_t *bus, void *data, uint8 type, uint16 ringid)
{
	...
	switch (type) {
		...
		case RING_MEM_ID:
			...
			dhdpcie_bus_wtcm8(bus, daddr32, *(uint8 *)data);
			break;
		...
		case RING_MAX_ITEMS:
			...
			dhdpcie_bus_wtcm16(bus, daddr32, (uint16) HTOL16(*(uint16 *)data));
			break;
		...
		case BUZZZ_HOSTMEM_SEGMENTS:
			...
			dhdpcie_bus_membytes(bus, TRUE, daddr32, (uint8*)data,
		...
		case H2D_IFRM_INDX_WR_BUF:
			long_data = HTOL64(*(uint64 *)data);
			daddr32   = bus->pcie_ipc_rings_daddr32;
			daddr32  += OFFSETOF(pcie_ipc_rings_t, ifrm_wr_haddr64);
			dhdpcie_bus_membytes(bus, TRUE, daddr32, (uint8*) &long_data, 8);
			break;
...
dhd_bus_cmn_readshared(dhd_bus_t *bus, void* data, uint8 type, uint16 ringid)
{
	...
	switch (type) {
		...
		case RING_RD_UPD:
			daddr32  = bus->ring_mem[ringid].rd_daddr32;
			*(uint16*)data = LTOH16(dhdpcie_bus_rtcm16(bus, daddr32));
			break;
...
static void dhd_buzzz_dump(bcm_buzzz_t *buzzz_p, void *buffer_p, char *page_p);
...
dhd_buzzz_dump(bcm_buzzz_t *buzzz_p, void *buffer_p, char *page_p)
{
	int i;
...
int /* Dump dongle buzzz log buffer using the pcie_ipc::buzzz_daddr32 */
dhd_buzzz_dump_dngl(dhd_bus_t *bus)
{
	...
	if (buzzz_daddr32 != 0U) {
		...
		dhdpcie_bus_membytes(bus, FALSE, buzzz_daddr32,
...
#ifdef BCM_ROUTER_DHD
	bytes += sprintf(p, "%16llu %12u : ", (*tcnt / cusec), diff);
#else
	bytes += sprintf(p, "%16llu %12u : ", (*tcnt / cusec), diff);
#endif // endif
...
int /* In stream mode, all logs are in host memory. */
dhd_buzzz_dump_dngl(dhd_bus_t *bus)
{
	uint32 indent = 0U;
	uint64 tcnt = 0;
	...
	printf("BUZZZ Streaming[%s] dump FW IPC rev %u, chip 0x%x %u\n"
		"\t Cycles Per Usec = %u\n"
...
#if !defined(BCM_ROUTER_DHD)
			if ((logs % 256) == 0) {
				OSL_CPU_RELAX(); OSL_DELAY(1000); /* kmsg overflow on BU */
			}
#endif /* ! BCM_ROUTER_DHD */
                ...
                void *params, int plen, void *arg, int len, int val_size)
{
	...
	case IOV_SVAL(IOV_HALT_DONGLE): /* halts ARM and PSM(s) for debug inspection */
	{
		...
		if (cpu_type != 0 && si_iscoreup(bus->sih) && int_val != 0) {
			...
			case ARMCA7_CORE_ID:
				...
				dhdpcie_bus_membytes(bus, TRUE, bus->dongle_ram_base,
			...
			bcmerror = dhd_macdbg_halt_psms(bus->sih, int_val, &bus->psm_run);
			bus->dhd->busstate = DHD_BUS_DOWN;
		}
		break;
	}
	...
	case IOV_GVAL(IOV_DUMP_DONGLE):
	{
		...
		bcmerror = dhd_macdbg_dump_dongle(bus->dhd, bus->sih, len, ddi, ddo);
		break;
	}
	...
	{
		strncpy(arg, bus->dhd->wlcore->fw_capabilities,
...
void dhd_bus_dump(dhd_pub_t *dhdp, struct bcmstrbuf *strbuf)
{
	...
	bcm_bprintf(strbuf,
		"%s %4s %4s %2s %4s %17s %4s %4s %6s %10s %9s %9s ",
		"Num:", "Offl", "Flow", "If", "Prio", ":Dest_MacAddress:", "Qlen", "CLen", "L2CLen",
		"Overflows", "RD", "WR");
 ...
 * Otherwise if we use HostToDev2Doorbell0 for HWA iDMA which can also be use by IDMA/IFRM too
...
/** doorbell ring function (HostToDev2Doorbell0) for IDMA/IFRM using dma channel2 */
void
...
dhdpcie_handle_mb_data(dhd_bus_t *bus)
{
	...
	if ((d2h_mb_data & PCIE_IPC_D2HMB_DEV_FWHALT))  {
		...
		/* debug_monitor should not dump d11 core when d11 core is down (noclk=1): */
		idx = si_coreidx(bus->sih);
	        if (si_setcore(bus->sih, D11_CORE_ID, 0) != NULL) {
			if (si_iscoreup(bus->sih)) {
				isup = TRUE;
			}
			si_setcoreidx(bus->sih, idx);
		...
		dhd_schedule_trap_log_dump(bus->dhd, isup);
	}
}
...
dhdpcie_tcm_valid(dhd_bus_t *bus)
{
	...
	pcie_ipc_daddr32 = LTOH32(dhdpcie_bus_rtcm32(bus, last_word_daddr32));
	...
	if ((rv = dhdpcie_bus_membytes(bus, FALSE, pcie_ipc_daddr32,
		(uint8 *)&pcie_ipc, PCIE_IPC_REV5_SZ)) < 0)
	{
		DHD_ERROR(("Failed to read PCIe IPC base struct with %d\n", rv));
		return FALSE;
	}
...
dhdpcie_ipc_rev_compatible(uint32 host_rev, uint32 dngl_rev)
{
	...
	if (PCIE_IPC_REV_IS_BCA(dngl_rev)) {
		if (host_rev >= dngl_rev) {
			...
			DHD_ERROR(("BCA PCIE IPC Rev failure:"
				" host_rev 0x%02x < dngl_rev 0x%02x\n", host_rev, dngl_rev));
			return FALSE;
		}
	} else {
...
dhdpcie_bus_read_pcie_ipc(dhd_bus_t *bus, daddr32_t *pcie_ipc_daddr32,
	pcie_ipc_t *pcie_ipc, pcie_ipc_rings_t *pcie_ipc_rings)
{
	uint32 pcie_ipc_size; /* based on dongle pcie ipc revision */
	daddr32_t daddr32 = 0U; /* pcie_ipc_t location in dongle memory */
	uint32 host_ipc_revision, dngl_ipc_revision;

	if (pcie_ipc == (pcie_ipc_t*)NULL) {
		DHD_ERROR(("%s: invoked without local pcie_ipc storage\n",
			__FUNCTION__));
		return BCME_BADARG;
	}

	if (pcie_ipc_daddr32 != (daddr32_t*)NULL) {
		*pcie_ipc_daddr32 = 0U;
	}
	...
	{   /* Locate the pcie_ipc_t structure in dongle's memory */
		       ...
		       !dhd_timeout_expired(&tmo))
		{
			...
			daddr32 = dhdpcie_bus_rtcm32(bus, last_word_daddr32);
			daddr32 = LTOH32(daddr32);
		}
		...
		{
			DHD_ERROR(("%s: pcie_ipc daddr32 0x%08x invalid. ram[0x%08x,0x%08x]\n",
				__FUNCTION__, daddr32, bus->dongle_ram_base, last_word_daddr32));
				return BCME_NOTREADY;
		}

		DHD_ERROR(("PCIe IPC addr (0x%08x) read took %u usec\n",
			daddr32, tmo.elapsed));
	}
	...
	dngl_ipc_revision = dhdpcie_bus_rtcm32(bus, daddr32);
	dngl_ipc_revision = LTOH32(dngl_ipc_revision) & PCIE_IPC_FLAGS_REVISION_MASK;

	host_ipc_revision = PCIE_IPC_REVISION;

	if (!(dhdpcie_ipc_rev_compatible(host_ipc_revision, dngl_ipc_revision))) {
		DHD_ERROR(("%s: PCIe IPC Revision: host %u incompatible with dngl %u\n",
			__FUNCTION__, host_ipc_revision, dngl_ipc_revision));
		return BCME_VERSION;
	}

	{   /* Copy pcie_ipc_t into local */
		int ret;
		...
		ret = dhdpcie_bus_membytes(bus, FALSE, daddr32,
		                           (uint8 *)pcie_ipc, pcie_ipc_size);
		if (ret < 0) {
			DHD_ERROR(("%s: Upload pcie_ipc daddr 0x%08x size %u FAILURE %d\n",
				__FUNCTION__, daddr32, pcie_ipc_size, ret));
			return ret;
		}
		...
		_PCIE_IPC_LTOH32(host_mem_len);
		_PCIE_IPC_LTOH32(host_mem_haddr64.lo);
		_PCIE_IPC_LTOH32(host_mem_haddr64.hi);
		// PAD[3]
		...
		if (PCIE_IPC_EXTN_PRESENT(dngl_ipc_revision)) {
			...
			_PCIE_IPC_LTOH32(fatal_logbuf_daddr32);
			// PAD[6]
		}

		DHD_ERROR(("PCIe IPC: addr 0x%08x sz %u, Rev host 0x%02x dngl 0x%02x\n"
			"\t\tFWID 0x%08x, flags 0x%08x, dcap1 0x%08x dcap2 0x%08x\n"
			"\t\ttrap 0x%08x assrt 0x%08x,0x%08x,%u cons 0x%08x fatal 0x%08x\n"
			...
			pcie_ipc->console_daddr32, pcie_ipc->fatal_logbuf_daddr32,
	...
	if (pcie_ipc_rings != (pcie_ipc_rings_t*)NULL) {
		...
		ret = dhdpcie_bus_membytes(bus, FALSE, pcie_ipc->rings_daddr32,
		                           (uint8 *)pcie_ipc_rings, pcie_ipc_rings_sz);
		if (ret < 0) {
			DHD_ERROR(("%s: Upload pcie_ipc_rings daddr 0x%08x size %u FAILURE %d\n",
				__FUNCTION__, pcie_ipc->rings_daddr32, pcie_ipc_rings_sz, ret));
			return ret;
		}
		...
		if (PCIE_IPC_EXTN_PRESENT(dngl_ipc_revision))
		{
			...
			_PCIE_IPC_RINGS_LTOH32(ifrm_wr_haddr64.lo);
			_PCIE_IPC_RINGS_LTOH32(ifrm_wr_haddr64.hi);
		}
		...
		if (pcie_ipc_rings->max_h2d_rings > (BCMPCIE_H2D_COMMON_MSGRINGS +
			...
			DHD_ERROR(("PCIe IPC: too many h2d rings %u flow rings %u, "
				"reduce to %u:%u\n",
		...
		DHD_ERROR(("PCIe IPC RINGS sz %u: ring_mem 0x%08x\n"
			"\t\th2d wr 0x%08x rd 0x%08x d2h wr 0x%08x rd 0x%08x\n"
			"\t\trings h2d %u d2h %u flows %u ifs %u\n",
		...
		// bcm_print_bytes("PCIe IPC Rings Info",
		//	(uchar *)pcie_ipc_rings, pcie_ipc_rings_sz);
...
dhdpcie_bus_init_pcie_ipc(dhd_bus_t *bus)
{
	...
	ret = dhdpcie_bus_read_pcie_ipc(bus,
		&bus->pcie_ipc_daddr32, &bus->pcie_ipc, &bus->pcie_ipc_rings);

	if (ret < 0) {
		DHD_ERROR(("%s : PCIe IPC upload FAILURE %d\n", __FUNCTION__, ret));
		return ret;
	}
	...
	DHD_ERROR(("PCIe IPC: d2h_sync_mode 0x%08x\n", bus->dhd->d2h_sync_mode));

	ASSERT(pcie_ipc->dma_rxoffset == 0U);
	dhd_prot_rx_dataoffset(bus->dhd, pcie_ipc->dma_rxoffset);
	...
	DHD_ERROR(("PCIe IPC: Max rings: h2d %d d2h %d\n",
	...
	DHD_ERROR(("PCIe IPC: %d bytes sized indices\n", bus->rw_index_sz));
	...
	/* iFRAM */
	if (IDMA_CAPABLE(bus) && bus->ifrm_enabled) {
		if (pcie_ipc->dcap1 & PCIE_IPC_DCAP1_IFRM) {
			bus->dhd->ifrm_enable = TRUE;
			pcie_ipc->hcap1 |= PCIE_IPC_HCAP1_IFRM;
			/* Disable dma_h2d_ring_upd_support when iFRM is enable */
			/* iFRM use h2d_ifrm_indx_wr_buf */
			bus->dhd->dma_h2d_ring_upd_support = FALSE;
			DHD_ERROR(("Host iFRM enable\n"));
		} else {
	...
	if (pcie_ipc->flags & PCIE_IPC_FLAGS_DMA_INDEX) {
		...
		if (pcie_ipc->flags & PCIE_IPC_FLAGS_2BYTE_INDICES) {

			bus->dhd->dma_d2h_ring_upd_support = TRUE;

			if (IFRM_ENAB(bus->dhd)) {
				bus->dhd->dma_h2d_ring_upd_support = FALSE;
				bus->ifrm_index_num_per_group = BCMPCIE_IFRM_64_INDEX_PER_GROUP;
			} else {
				bus->dhd->dma_h2d_ring_upd_support = TRUE;
			}
		}
		DHD_ERROR(("PCIe IPC: DMAing indices: H2D %d D2H %d\n",
	...
	/* Allocate DMA index WR buffer for iFRM */
	if (IFRM_ENAB(bus->dhd)) {
		dma_indx_wr_buf = dhd_prot_dma_indx_init(bus->dhd, bus->rw_index_sz,
			H2D_IFRM_INDX_WR_BUF, pcie_ipc_rings->max_flowrings);

		if (dma_indx_wr_buf != BCME_OK) {
			DHD_ERROR(("%s: Failed to alloc memory for Implicit DMA\n",
					__FUNCTION__));
			bus->dhd->ifrm_enable = FALSE;
			DHD_ERROR(("%s(%d): Set ifrm_enable FALSE\n", __FUNCTION__, __LINE__));
		}
	}

	DHD_ERROR(("PCIe IPC: WI Formats TxP %u RxP %u TxC %u RxC %u\n",
...
#ifdef BCMHWA
	/* Remove it when we don't support 43684a0 HWA anymore */
	if (BCM43684_CHIP(bus->sih->chip) && CHIPREV(bus->sih->chiprev) <= 1)
		dhd_prot_set_hwa_rev(bus->dhd, 128);
	else
		dhd_prot_set_hwa_rev(bus->dhd, 129);
	...
	if (dhd_prot_preinit(bus->dhd, use_haddr64, host_physaddrhi,
	        ...
	        pcie_ipc->flags))
	{
		DHD_ERROR(("Host not capable of ACWI or CWI formats\n"));
		ASSERT(0);
		return BCME_ERROR;
	}
	...
	DHD_ERROR(("PCIe IPC: RxPost buf_len %u host_physaddrhi 0x%08x\n",
...
int dhd_bus_init(dhd_pub_t *dhdp, bool enforce_mutex)
{
	...
	if (ret < 0) {
		DHD_ERROR(("%s : PCIe IPC init failed \n", __FUNCTION__));
		return ret;
	}
...
dhdpcie_bus_clear_pcie_ipc_addr(dhd_bus_t *bus)
{
	...
	uint32 val = 0;
	last_word_daddr32 = bus->dongle_ram_base + bus->ramsize - 4;
	dhdpcie_bus_membytes(bus, TRUE, last_word_daddr32, (uint8 *)&val, sizeof(val));
}
...
dhdpcie_chipmatch(uint16 vendor, uint16 device)
{
	...
	if ((device == BCM43684_D11AX_ID) || (device == BCM43684_D11AX2G_ID) ||
		(device == BCM43684_D11AX5G_ID) || (device == BCM43684_CHIP_ID))
		return 0;
