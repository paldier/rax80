 ...
 *
 * $Id: dhd_linux.c 767034 2018-08-24 17:11:49Z $
 */
...
#define RXBUFSZ         DHD_RX_PKT_BUFSZ /* includes BCMEXTRAHDROOM */
#endif /* CTFPOOL */
...
#ifdef DHD_WMF
static void dhd_set_wmf_handler(void *handle, void *event_info, u8 event);
#endif /* DHD_WMF */
static void dhd_fwtrap_handler(void *handle, void *event_info, u8 event);
static void dhd_dtrace_handler(void *handle, void *event_info, u8 event);
#if defined(OEM_ANDROID) && defined(CONFIG_IPV6)
	...
	list_for_each_entry_safe(sta, next, &ifp->sta_list, list) {
		if (!memcmp(sta->ea.octet, ea, ETHER_ADDR_LEN)) {
			...
			if (ifp->wmf.wmf_enable && sta->src_ip &&
				emfc_remove_sta(ifp->wmf.wmf_instance->emfci, sta, sta->src_ip))
...
static void dhd_rpcth_watchdog(ulong data)
{
	dhd_info_t *dhd = (dhd_info_t *)data;
#else
static void dhd_rpcth_watchdog(struct timer_list *t)
{
...
dhd_set_multicast_list(struct net_device *dev)
{
	...
	dhd_deferred_schedule_work(dhd->dhd_deferred_wq, (void *) dev,
...
#if defined(BCM_DHDHDR) && defined(PCIE_FULL_DONGLE)
	/* Multicast/EAPOL packets from CPU may share the same data, we need to make a copy
...
#endif // endif
#ifdef BCM_NBUFF_WLMCAST
		/* only for skb, need to do pktdup_cpy */
		(IS_SKBUFF_PTR(pktbuf)) &&
#endif // endif
		TRUE) {
		void *npkt;
		struct ether_header *eh;
...
dhd_dev_lkup_flush_queues(struct net_device *dev)
{
	...
	for (item = dll_head_p(dlist); !dll_end(dlist, item); item = next) {
		...
		queue = container_of(item, dhd_dev_lkup_queue_t, node);
...
#endif /* KERNEL_VERSION >= 2.6.36 && !defined(BCM_DHD_RUNNER) */
{
	...
	return net_dev_collect_stats(net);
#else /* !BCM_DHD_RUNNER */
	dhd_info_t *dhd = DHD_DEV_INFO(net);
	dhd_if_t *ifp;
	int ifidx;
	...
	uint8 *mac, uint8 bssidx, bool need_rtnl_lock, const char *dngl_name)
{
	...
	if (ifidx == 0)
		...
		ifp->net->destructor = wl_netdev_free;
#else
	ifp->net->destructor = free_netdev;
#endif /* WL_CFG80211 */
...
#if (defined(BCM_ROUTER_DHD) && defined(HNDCTF))
			if (dhdinfo->cih)
				ctf_dev_unregister(dhdinfo->cih, net);
#endif /* BCM_ROUTER_DHD && HNDCTF */
 ...
 */
static bool dhd_bea_is_valid_file(char *_bea_file_path)
{
	struct file *filep;
	mm_segment_t fs;
	bool ret = FALSE;
...
dhd_bea_read_fw_syms(struct file *filep, uint32 *ramstart, uint32 *rodata_start, uint32 *rodata_end)
{
	...
	ret = kernel_read(filep, ramstart, sizeof(uint32), pos);
	if (ret == sizeof(uint32)) {
		ret = kernel_read(filep, rodata_start, sizeof(uint32), pos);
	}

	if (ret == sizeof(uint32)) {
		ret = kernel_read(filep, rodata_end, sizeof(uint32), pos);
	}
...
dhd_event_log_alloc(dhd_pub_t *dhdpub,
	char *_bea_file_path, char *_logstrs_path, char *_st_str_file_path,
	char *_map_file_path, char *_rom_st_str_file_path, char *_rom_map_file_path)
{
	dhd_info_t *dhd;
	...
	dhd_init_logstrs_array(&dhd->event_data,
		_bea_file_path != NULL ? _bea_file_path : _logstrs_path,
		_bea_file_path != NULL);
	dhd_init_static_strs_array(&dhd->event_data,
		_bea_file_path != NULL ? _bea_file_path : _st_str_file_path,
		_bea_file_path != NULL ? NULL : _map_file_path,
		TRUE);
	dhd_init_static_strs_array(&dhd->event_data,
		_bea_file_path != NULL ? _bea_file_path : _rom_st_str_file_path,
		_bea_file_path != NULL ? NULL : _rom_map_file_path,
		FALSE);
...
dhd_attach(osl_t *osh, struct dhd_bus *bus, uint bus_hdrlen)
{
...
#if defined(BCM_GMAC3) && defined(DHD_DPSTA) && !defined(BCM_NO_WOFA)
	char *var;
#endif /* DHD_DPSTA */
...
#if defined(BCM_GMAC3) && defined(DHD_DPSTA) && !defined(BCM_NO_WOFA)
	/* We should prevent to enable GMAC3 when DPSTA was enabled. DPSTA will add
	 * duplicated interface because GMAC3 may forward pkt in different radios.
	 */
	var = getvar(NULL, "dpsta_ifnames");
	if (var != NULL && var[0] != '\0') {
		DHD_ERROR(("dpsta_ifnames=%s, skip fwder_attach\n", var));
		goto skip_fwder_attach;
	}
#endif // endif
...
#ifdef CTFPOOL
	{
		...
		if (CTF_ENAB(dhd->cih) && (osl_ctfpool_init(dhd->pub.osh,
			poolsz, RXBUFSZ) < 0)) /* RXBUFSZ includes BCMEXTRAHDROOM */
		{
...
#else
	timer_setup(&dhd->timer, dhd_watchdog);
#endif /* LINUX_VERSION_CODE < KERNEL_VERSION(4, 15, 0) */
		...
		if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_GET_VAR, buf, sizeof(buf),
			...
			if (buf[0] == 1) {
				...
				*/
				...
				if (FW_SUPPORTED(dhd->wlcore, rsdb)) {
...
dhd_preinit_ioctls(dhd_pub_t *dhd)
{
	...
	/* get a capabilities from firmware */
	{
		...
		bcm_mkiovar("cap", 0, 0, dhd->wlcore->fw_capabilities, cap_buf_size - 1);
		if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_GET_VAR, dhd->wlcore->fw_capabilities,
...
#else
	/* get a capabilities from firmware */
	memset(dhd->wlcore->fw_capabilities, 0,
		sizeof(dhd->wlcore->fw_capabilities));
	bcm_mkiovar("cap", 0, 0, dhd->wlcore->fw_capabilities,
		sizeof(dhd->wlcore->fw_capabilities));
	if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_GET_VAR, dhd->wlcore->fw_capabilities,
		sizeof(dhd->wlcore->fw_capabilities), FALSE, 0)) < 0) {
		DHD_ERROR(("%s: Get Capability failed (error=%d)\n",
			__FUNCTION__, ret));
		goto done;
	}
...
#else
	timer_setup(&pub->info->rpcth_timer), dhd_rpcth_watchdog);
#endif /* LINUX_VERSION_CODE < KERNEL_VERSION(4, 15, 0) */
...
#define CUSTOM_INTR_WIDTH 100
	...
	if (FW_SUPPORTED(dhd->wlcore, nan)) {
		feature_set |= WIFI_FEATURE_NAN;
		/* NAN is essentail for d2d rtt */
		if (FW_SUPPORTED(dhd->wlcore, rttd2d))
 ...
 */
static void
socsram_write_to_file_no_large_buf(dhd_pub_t *pub)
{
	uint32 memsize;
	struct {
		uint32 bp_address;
	...
	if (dhd_bus_iovar_op(pub, "memsize", NULL, 0, &memsize, sizeof(memsize), FALSE) < 0) {
		goto the_end;
	}

	fp = open_file(SOCRAM_DUMP_FILE_NAME, SOCRAM_DUMP_FILE_FLAGS);
	if (fp == NULL) {
		goto the_end;
	}
	...
	while (memsize)	{
		...
		if (dhd_bus_iovar_op(pub, "membytes", &m, sizeof(m), buf, MEMBLOCK, FALSE) < 0) {
...
dhd_mem_dump(void *handle, void *event_info, u8 event)
{
	...
	if (dump->buf == NULL) {
		...
		socsram_write_to_file_no_large_buf(&dhd->pub);
...
void dhd_schedule_trap_log_dump(dhd_pub_t *dhdp, bool isup)
{
	bool *p_isup = (bool *)MALLOCZ(dhdp->osh, sizeof(*p_isup));
	if (p_isup == NULL) {
		DHD_ERROR(("%s: malloc fail for p_isup\n", __FUNCTION__));
		return;
	}
	*p_isup = isup;

	if (dhd_deferred_schedule_work(dhdp->info->dhd_deferred_wq, p_isup,
		DHD_WQ_WORK_FWDUMP, dhd_fwtrap_handler, DHD_WORK_PRIORITY_HIGH) != DHD_WQ_STS_OK) {
		MFREE(dhdp->osh, p_isup, sizeof(*p_isup));
	}
	dhd_deferred_work_set_skip(dhdp->info->dhd_deferred_wq,
		DHD_WQ_WORK_FWDUMP, TRUE);
}
...
void dhd_schedule_dtrace(dhd_pub_t *dhdp, uint8 *event_data, uint32 datalen)
{
	int res;
...
static int
_dhd_macdbg_dump_name(dhd_info_t *dhd, char* dumpname, char *dumpbuf)
{
	dhd_pub_t *dhdp = &dhd->pub;
...
static void
dhd_fwtrap_handler(void *handle, void *event_info, u8 event)
{
	dhd_info_t *dhd = handle;
	char *dumpbuf = NULL;
	dhd_pub_t *dhdp = &dhd->pub;
	...
	(void) dhd_wl_ioctl_set_intiovar(dhdp, "bus:disconnect", 102, WLC_SET_VAR, TRUE, 0);

	/* PSMr dump */
	if (_dhd_macdbg_dump_name(dhd, "mac", dumpbuf) == BCME_OK) {
		dumpnum++;
	}

	/* PSMr1 dump */
	if (_dhd_macdbg_dump_name(dhd, "mac1", dumpbuf) == BCME_OK) {
		dumpnum++;
	}

	/* PSMx dump */
	if (_dhd_macdbg_dump_name(dhd, "macx", dumpbuf) == BCME_OK) {
		dumpnum++;
	}

	/* sctpl dump */
	if (_dhd_macdbg_dump_name(dhd, "sctpl", dumpbuf) == BCME_OK) {
		dumpnum++;
	}

	/* scpsm dump */
	if (_dhd_macdbg_dump_name(dhd, "scpsm", dumpbuf) == BCME_OK) {
		dumpnum++;
	}

	/* ratelinkmem dump */
	if (_dhd_macdbg_dump_name(dhd, "ratelinkmem", dumpbuf) == BCME_OK) {
		dumpnum++;
	}

	/* physvmp dump */
	if (_dhd_macdbg_dump_name(dhd, "physvmp", dumpbuf) == BCME_OK) {
		dumpnum++;
	}

	/* rxbm dump */
	if (_dhd_macdbg_dump_name(dhd, "rxbm", dumpbuf) == BCME_OK) {
		dumpnum++;
	}

	/* bmc dump */
	if (_dhd_macdbg_dump_name(dhd, "bmc", dumpbuf) == BCME_OK) {
		dumpnum++;
	}

	/* peruser dump */
	if (_dhd_macdbg_dump_name(dhd, "peruser", dumpbuf) == BCME_OK) {
		dumpnum++;
	}

	DHD_ERROR(("%s: dump signature: %04x, dumpnum: %d, dtrace_cur_fidx: %1x\n",
		__FUNCTION__, dhd->dump_signature, dumpnum, dhd->dtrace_cur_fidx));
...
exit:
	MFREE(dhdp->osh, event_info, sizeof(bool));
	dhd_deferred_work_set_skip(dhd->dhd_deferred_wq,
		DHD_WQ_WORK_FWDUMP, FALSE);
#ifdef BCM_ROUTER_DHD
...
#define DTRACE_MAX_FILESIZE	(500 * 1024)	/* 500KB */
#define DTRACE_MAX_FILENUM	2		/* keep it power of 2 */

static void
dhd_dtrace_handler(void *handle, void *event_info, u8 event)
{
	dhd_info_t *dhd = handle;
	dhd_pub_t *dhdp = &dhd->pub;
	char *dumpbuf = NULL;
	...
	while (datalen < dtrace_wq->datalen) {
		...
		bcm_bprintf(b, "[%d] %01X ", i, dtrace_rec->id);
...
exit:
	...
	dhd_deferred_work_set_skip(dhd->dhd_deferred_wq,
		DHD_WQ_WORK_DTRACE, FALSE);
}

#ifdef BCMHWA
static void
dhd_hwa_event_handler(void *handle, void *event_info, u8 event)
{
	dhd_info_t *dhd = handle;
	dhd_pub_t *dhdp = &dhd->pub;
...
dhd_hwa_event(dhd_pub_t *dhdp)
{
	...
	dhd_deferred_work_set_skip(dhdp->info->dhd_deferred_wq,
		DHD_WQ_WORK_HWA, TRUE);
}
#endif /* BCMHWA */

#ifdef DHD_WMF
static void
dhd_set_wmf_handler(void *handle, void *event_info, u8 event)
{
	dhd_info_t *dhd = handle;
...
dhd_schedule_wmf_bss_enable(dhd_pub_t *dhdp, uint32 ifidx)
{
	...
	*ifidx_p = ifidx;
	dhd_deferred_schedule_work(dhdp->info->dhd_deferred_wq, (void *)ifidx_p,
		DHD_WQ_WORK_SET_WMF, dhd_set_wmf_handler, DHD_WORK_PRIORITY_LOW);
}
			...
			list_for_each_entry_safe(sta, next, &ifp->sta_list, list) {
				if (!memcmp(sta->ea.octet, mac, ETHER_ADDR_LEN)) {
					...
					DHD_INFO(("found: dhd_client_get_info:sta->ea:%pM \r\n",
							&(sta->ea)));
#if !defined(CONFIG_BCM963158) & !defined(CONFIG_BCM96846) & !defined(CONFIG_BCM96836)
			switch (dhd_flowid_get_type(&(ifp->info->pub), sta, priority)) {
 ...
 */
void dhd_wmf_update_if_stats(void *dhd_p, int ifidx, int wmf_action,
	unsigned int pktlen, bool frombss)
{
	...
	if (frombss) {
		if (wmf_action == WMF_TAKEN) {
			...
			ifp->stats.rx_packets++;
			ifp->stats.rx_bytes += pktlen;
#if defined(CONFIG_BCM_KF_EXTSTATS) && defined(BCM_WL_EXTSTATS)
