 ...
 *
 * $Id: services.c 766252 2018-07-27 10:33:00Z $
 */
...
mcpd_conf(void)
{
	...
	if (!nvram_match("router_disable", "1")) {
        if (nvram_match("wan_proto","pppoe"))
            proxy_ifname = nvram_safe_get("pppoe_ifname");
        else
    ...
    if (nvram_match("iptv_enabled", "1")) {
        ...
        fprintf(fp, "igmp-snooping-interfaces %s %s\n", nvram_safe_get("lan_ifname"), nvram_safe_get("lan1_ifname"));
        if (nvram_match("wan_proto","pppoe"))
        ...
        char vlanrule[10][128];
        int num_vlan_rule = 0;
        char *var;
        int i;

        if ((var = nvram_safe_get("vlan_rule_enable")) != NULL) {
            num_vlan_rule = getTokens(var, " ", vlanrule, 10);
            if (num_vlan_rule > 1) {
                int only_internet_vlan = 1;

                for (i = 1; i < num_vlan_rule; i++) {
                    if (strcmp(vlanrule[i], "1") == 0) {
                        only_internet_vlan = 0;
                        break;
                    }
                }
                if (only_internet_vlan == 1)
                    num_vlan_rule = 1;
            }
        }
        ...
        if (num_vlan_rule == 1) {
            ...
            fprintf(fp, "igmp-mcast-interfaces %s\n", proxy_ifname);
        } else if (num_vlan_rule > 1) {
            char br_group[256] = {0};
            char br_name[16] = {0};
            char *vlan_number[10];
            char vlan_group[256] = {0};
            char vlan_name[16] = {0};

            fprintf(fp, "igmp-proxy-interfaces\n");
            //snooping
            strncpy(br_group, nvram_safe_get("lan_ifname"), sizeof(br_group) - 1);
            strcat(br_group, " ");
            for (i = 1; i < num_vlan_rule; i++) {
                memset(br_name, 0x0, sizeof(br_name));
                snprintf(br_name, sizeof(br_name), "lan%d_ifname", i);
                strncat(br_group, nvram_safe_get(br_name), strlen(nvram_safe_get(br_name)));
                if (i < (num_vlan_rule - 1))
                    strcat(br_group, " ");
            }
            fprintf(fp, "igmp-snooping-interfaces %s\n", br_group);

            //mcast
            split(vlan_number, nvram_safe_get("vlan_id"), " ");
            for (i = 1; i < num_vlan_rule; i++) {
                memset(vlan_name, 0x0, sizeof(vlan_name));
                snprintf(vlan_name, sizeof(vlan_name), "vlan%s", vlan_number[i]);
                strncat(vlan_group, vlan_name, strlen(vlan_name));
                if (i < (num_vlan_rule - 1))
                    strcat(vlan_group, " ");
            }
            fprintf(fp, "igmp-mcast-interfaces %s\n", vlan_group);
        }
    } else if (nvram_match("igmp_proxying_enable", "1")) {
        if (nvram_match("bt_igmp_enable","1"))
            fprintf(fp, "igmp-proxy-interfaces %s %s\n", nvram_safe_get("wan0_ifname"), proxy_ifname);
        else
            fprintf(fp, "igmp-proxy-interfaces %s\n", proxy_ifname);
        fprintf(fp, "igmp-snooping-interfaces %s\n", nvram_safe_get("lan_ifname"));
        if (nvram_match("bt_igmp_enable","1"))
            fprintf(fp, "igmp-mcast-interfaces %s %s\n", nvram_safe_get("wan0_ifname"), proxy_ifname);
        else
            fprintf(fp, "igmp-mcast-interfaces %s\n", proxy_ifname);
    } else {
        fprintf(fp, "igmp-proxy-interfaces\n");
    ...
    fprintf(fp, "igmp-force-query 1\n");
    if (nvram_match("ft_igmp_enable", "1"))
         fprintf(fp, "igmp-ignore-asm-on-ssm 1\n");
...
int start_bsd(void)
{
...
#ifdef BCA_HNDROUTER
    char *SmartConnectBoth5GInterface;
    if (isTriBand()) {
        SmartConnectBoth5GInterface = nvram_get("wl_5g_bandsteering");
        if(SmartConnectBoth5GInterface[0] == '1')               //5G 5G2
            sprintf(command, "%s %s",WLAN_N_IF_NAME_NUM, WLAN_5G_2_IF_NAME_NUM);
        else                                                    //2G 5G 5G2
            sprintf(command, "%s %s %s", WLAN_IF_NAME_NUM, WLAN_N_IF_NAME_NUM, WLAN_5G_2_IF_NAME_NUM);
        nvram_set("gbsd_ifnames", command);
    } else {
        sprintf(command, "%s %s", WLAN_IF_NAME_NUM, WLAN_N_IF_NAME_NUM);
        nvram_set("gbsd_ifnames", command);
    }
#else /* !BCA_HNDROUTER */
    if (isTriBand())
        ...
        nvram_set("gbsd_ifnames", "eth1 eth2");
#endif /* !BCA_HNDROUTER */        
...
#ifdef BCA_HNDROUTER
    //if(SmartConnectBoth5GInterface[0] == '0')   //2G
    {
        sprintf(command, "wl -i %s bssload 0", WLAN_IF_NAME_NUM);
        system(command);
    }
    sprintf(command, "wl -i %s bssload 0", WLAN_N_IF_NAME_NUM);
    system(command);
#if defined(DUAL_TRI_BAND_HW_SUPPORT)  
    sprintf(command, "wl -i %s bssload 0", WLAN_5G_2_IF_NAME_NUM);
    system(command);
#endif
#else /* !BCA_HNDROUTER */
    ...
    if(nvram_match("debug_gbsd_enable", "1"))
    {
        ...
        sleep(1);
#if 0 //defined(AX6000)
        nvram_set("bsd_msglevel","0x13");
        system("/usr/sbin/bsd > /tmp/gbsd_debug.fifo");
#else
        system("/bin/gbsd > /tmp/gbsd_debug.fifo");
#endif /*AX6000*/
    }
    else
    {
...
#ifdef BCA_HNDROUTER
    sprintf(command, "wl -i %s bssload 1", WLAN_IF_NAME_NUM);
    system(command);
    sprintf(command, "wl -i %s bssload 1", WLAN_N_IF_NAME_NUM);
    system(command);
#if defined(DUAL_TRI_BAND_HW_SUPPORT)  
    sprintf(command, "wl -i %s bssload 1", WLAN_5G_2_IF_NAME_NUM);
    system(command);
#endif
#else /* !BCA_HNDROUTER */
...
#endif /* (LINUX_2_6_36 || BCA_HNDROUTER) && __CONFIG_TREND_IQOS__ */
...
*/
	...
	start_toads();
	    ...
	    start_bsd();
#else
		if(nvram_match("enable_band_steering", "1") && nvram_match("wlg_wlanstate", "Enable"))
	    start_bsd();
#endif
#ifdef __CONFIG_SAMBA__
#if defined(LINUX_2_6_36)
