...
static int cpu_queue_id_get(bdmf_object_handle cpu_obj, int radio_idx)
{
	 ...
	 */
	rdpa_cpu_num_queues_get(cpu_obj, &num_queues);
...
dhd_runner_cfg_cpu_queue(struct dhd_runner_hlp *dhd_hlp, int init)
{
	...
	if (!cpu_queue_size)
	{
	    ...
	    rdpa_cpu_index_get(dhd_hlp->cpu_obj, &cpu_port);
	    ...
	    rdpa_cpu_rxq_flush_set(dhd_hlp->cpu_obj, dhd_hlp->cpu_queue_id, 1);
	}
	else
	{
#ifdef XRDP
	...
	if (!cpu_queue_size)
	{
	    ...
	    rdpa_dhd_helper_cpu_data_set(dhd_hlp->dhd_helper_obj, NULL);
#endif
	    goto exit;
	}
...
dhd_runner_init(dhd_runner_hlp_t *dhd_hlp, struct pci_dev *pci_dev)
{
	...
	rdpa_dhd_helper_init_cfg_set(dhd_helper_attr, &dhd_hlp->dhd_init_cfg);
	...
	dhd_runner_ring_cfg_t *ring_cfg)
{
	...
	if (BcmMemReserveGetByName(dhd_name,
	    (void **)&flowmgr->hw_mem_addr, &flowmgr->hw_mem_size) == 0) {
